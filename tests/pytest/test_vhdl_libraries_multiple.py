...
@pytest.mark.skipif(
    os.getenv("SIM", "ghdl") not in ["ghdl", "questa", "modelsim", "xcelium"],
    reason="Skipping test since only GHDL, Questa/ModelSim and Xcelium are supported",
)
def test_toplevel_library():
    ...
    for lib in ["e", "d", "c", "b", "a"]:
        runner.build(
            ...
            build_dir=str(src_path / "pytest" / "sim_build"),
        )
